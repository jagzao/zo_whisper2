...
def test_frame_vision_blocked_when_frame_upload_off_for_remote(tmp_path, monkeypatch):
    settings = _settings(frame_descriptions=True, allow_image_upload=False)
    fake_provider = MagicMock()
    fake_provider.provider_type = LLMProviderType.REMOTE
    service = AIEnrichmentService(fake_provider, PrivacyGuard(settings), settings)
    monkeypatch.setattr(handler_module, "_ai_service", service)
...
def test_frame_vision_allowed_when_all_flags_permissive(tmp_path, monkeypatch):
    settings = _settings(frame_descriptions=True, allow_image_upload=True, allow_external_llm=True)
    fake_provider = MagicMock()
    fake_provider.provider_type = LLMProviderType.REMOTE
...
def test_scan_documents_allows_vision_client_for_internal_project(tmp_path, monkeypatch):
    settings = _settings(allow_external_llm=True, frame_descriptions=True, allow_image_upload=True)
    fake_provider = MagicMock()
    fake_provider.provider_type = LLMProviderType.REMOTE
    service = AIEnrichmentService(fake_provider, PrivacyGuard(settings), settings)
    monkeypatch.setattr(handler_module, "_ai_service", service)
...
def test_scan_documents_blocks_vision_client_when_frame_descriptions_off(tmp_path, monkeypatch):
    """Regression: _scan_documents' vision-client gate previously only
    checked PrivacyGuard, so a document's embedded images could reach a
    remote vision model even with FRAME_DESCRIPTIONS=false."""
    settings = _settings(allow_external_llm=True, frame_descriptions=False, allow_image_upload=True)
    fake_provider = MagicMock()
    fake_provider.provider_type = LLMProviderType.REMOTE
    service = AIEnrichmentService(fake_provider, PrivacyGuard(settings), settings)
    monkeypatch.setattr(handler_module, "_ai_service", service)
