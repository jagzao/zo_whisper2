...
- `ALLOW_FRAME_UPLOAD=false` additionally blocks a remote call even when
  the above two would otherwise allow it — text generation and image
  upload are separate permissions for a remote provider.
...
class AIEnrichmentService:
    ...
    def summarize(
        ...
        return self._provider.generate_summary(redact_secrets(text), system_prompt=system_prompt)
    ...
    def describe_frame_with_prompt(
        ...
        *,
        ...
        return self._provider.describe_frame_with_prompt(
            image_path, prompt, max_tokens=max_tokens, temperature=temperature
        )
    ...
    def describe_frames_for_tutorial(
        ...
        return self._provider.describe_frames_for_tutorial(frames_dir, mapping)
    ...
    def _check_frame_policy(self, project_config: dict[str, Any] | None) -> None:
        ...
        self._guard.check(self._provider, project_config)
        if self._provider.provider_type is LLMProviderType.REMOTE and not self._settings.allow_frame_upload:
            raise ExternalLLMBlockedError(
                "ALLOW_FRAME_UPLOAD is disabled — refusing to send frame images to a remote provider."
            )
