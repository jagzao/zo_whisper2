...
class ClientAHandler:
    ...
    def process(self, transcription_data, original_file_path, project_config: dict = None):
        """
        Procesa una transcripción para el proyecto ClientA.
        Crea la estructura: meetings/{Date}_{Name}/[transcripcion.md, resumen.md]
        """
        try:
            filename = original_file_path.stem
            clean_name = filename.replace("clienta_", "").replace("py_clienta", "")
            ...
            logger.info(f"[CLIENTA] Transcripción guardada en: {transcription_path}")
            ...
            if not resumen_path.exists():
                if _LLM_AVAILABLE:
                    try:
                        ...
                        logger.info(f"[CLIENTA] Resumen generado por LLM: {resumen_path}")
                    except Exception as e:
                        logger.warning(f"[CLIENTA] LLM falló ({e}), usando plantilla vacía")
            ...
            logger.error(f"[CLIENTA] Error procesando {original_file_path.name}: {e}")
            return False
